...
sealed class ShellContext {
    ...
    data class Inferred(
        val booleanStatement: BooleanStatement = BooleanStatement.StringOperator.Contains("PATH".environmentVar(), ("C:".strSingle())),
    ) : ShellContext()
}
                    ...
                    "./gradlew --stacktrace --build-cache " +
                        (
                            gradleCredentials?.let {
                                val userProp = "-D$gradleUserProperty=\\\"\\\${${it.usernameVariable.value}}\\\" "
                                val passProp = "-D$gradlePasswordProperty=\\\"\\\${${it.passwordVariable.value}}\\\" "
                                userProp + passProp
                            } ?: ""
                            ) +
