...
import com.code42.jenkins.pipelinekt.core.step.Sequence
                    ...
                    Stage.Matrix(
                            ...
                            matrixBody = MatrixBody(
                                    ...
                                    stages = listOf(Stage.Steps(
                                            ...
                                            steps = Sequence(listOf(PipelineMethodInvocation("nested_stage"))),
                                            post = Post(
                                                    always = Sequence(emptyList()),
                                                    cleanup = Sequence(emptyList()),
                                                    success = Sequence(emptyList())))),
                                    post = Post(
                                            always = Sequence(listOf(Sh("./matrix-always.sh".strDouble()))),
                                            cleanup = Sequence(emptyList()),
                                            success = Sequence(emptyList()))),
                            post = Post(
                                    always = Sequence(listOf(Sh("./stage-always.sh".strDouble()))),
                                    cleanup = Sequence(emptyList()),
                                    success = Sequence(emptyList())))),
                methods = listOf(PipelineMethod("nested_stage", Sequence(listOf(Sh("./call.sh".strDouble()))))),
                post = Post(
                        always = Sequence(emptyList()),
                        cleanup = Sequence(listOf(CleanWs)),
                        success = Sequence(emptyList())))
