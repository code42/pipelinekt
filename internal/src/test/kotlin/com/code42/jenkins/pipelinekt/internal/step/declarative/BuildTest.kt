...
class BuildTest : GroovyScriptTest() {
    ...
    @Test fun buildOneParameter() {
        val expected = "build([\n" +
            ...
            "])\n"
        Build(
            "my_downstream_job".strDouble(),
            listOf(StringBuildParameterValue("COMMIT_HASH".strSingle(), "GIT_COMMIT".environmentVar())),
            true.boolVar(),
        ).toGroovy(writer)
        assertEquals(expected, out.toString())
    }
    ...
    @Test fun buildManyParameters() {
        val expected = "build([\n" +
            "${writer.indentStr}job: \"my_downstream_job\",\n" +
            "${writer.indentStr}parameters: [[\$class: 'hudson.model.StringParameterValue', name: 'COMMIT_HASH', value: env.GIT_COMMIT],\n" +
            "${writer.indentStr}${writer.indentStr}[\$class: 'hudson.model.StringParameterValue', " +
            "name: 'EXT_BUILD_NUMBER', value: env.EXT_BUILD_NUMBER],\n" +
        ...
        Build(
            "my_downstream_job".strDouble(),
            listOf(
                StringBuildParameterValue("COMMIT_HASH".strSingle(), "GIT_COMMIT".environmentVar()),
                StringBuildParameterValue("EXT_BUILD_NUMBER".strSingle(), "EXT_BUILD_NUMBER".environmentVar()),
                StringBuildParameterValue("BRANCH_NAME".strSingle(), "GIT_BRANCH".environmentVar()),
            ),
            true.boolVar(),
        ).toGroovy(writer)
        assertEquals(expected, out.toString())
