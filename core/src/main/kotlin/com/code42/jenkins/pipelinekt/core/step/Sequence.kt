...
data class Sequence(val steps: List<Step>) : DeclarativeStep {
    ...
    override fun andThen(next: Step): Step = when (next) {
        is Sequence -> this.copy(steps = steps + next.steps)
        is Void -> this
        else -> this.copy(steps = steps + next)
    }

    override fun precededBy(previous: Step): Step = when (previous) {
    ...
    fun flatten(): Step = steps.fold<Step, Step>(Void, { flattendSteps, step ->
        when (step) {
